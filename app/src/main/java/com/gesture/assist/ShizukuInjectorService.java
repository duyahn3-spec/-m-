...
public class ShizukuInjectorService extends Service {
    ...
    private void executeAllCommands() {
        if (!ready) {
            Toast.makeText(this, "Shizuku chưa sẵn sàng hoặc chưa có quyền", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public void onCreate() {
        ...
        if (Shizuku.pingBinder()) {
            if (Shizuku.checkSelfPermission() == 0) {
                ready = true;
                Toast.makeText(this, "Shizuku ready (shell via Runtime.exec)", Toast.LENGTH_SHORT).show();
            } else {
